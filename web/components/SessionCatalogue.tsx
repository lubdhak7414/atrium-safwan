...
export function SessionCatalogue({
  ...
  const filtered = useMemo(() => sessions.filter((session) => {
    ...
    return (discipline === 'all' || session.discipline === discipline)
      ...
      && (!date || localDate === date);
  }), [date, discipline, sessionType, sessions]);
  ...
  return (
    <section className="data-panel" aria-labelledby={`${title.toLowerCase().replaceAll(' ', '-')}-title`}>
      <div className="section-heading">
        <div>
          <h2 id={`${title.toLowerCase().replaceAll(' ', '-')}-title`}>{title}</h2>
          <p className="muted">{filtered.length} of {sessions.length} sessions shown.</p>
        </div>
      </div>
      <div className="filter-bar" aria-label="Catalogue filters">
        ...
        <button type="button" className="filter-reset" onClick={() => { setDiscipline('all'); setSessionType('all'); setDate(''); }}>CLEAR</button>
      </div>
      ...
      {state === 'ready' && filtered.length > 0 && (
        <div className="table-scroll">
          <table>
            <thead><tr><th>ID / DISCIPLINE</th><th>TYPE</th><th>WHEN</th><th>ROOM</th><th>SEAT FEE</th><th>PLACES</th>{bookingRole && <th>ACTION</th>}</tr></thead>
            <tbody>{filtered.map((session) => {
              ...
              return (
                <tr key={session.id}>
                  ...
                  {bookingRole && <td>{booked ? <button type="button" disabled={actionId === session.id} onClick={() => void cancel(session)}>CANCEL BOOKING</button> : <button type="button" disabled={full || actionId === session.id} onClick={() => void enrol(session)}>{full ? 'FULL' : 'BOOK SESSION'}</button>}</td>}
                </tr>
              );
            })}</tbody>
          </table>
      ...
      {promotedOnly && <p className="table-note">Promoted sessions are selected by an administrator.</p>}
    </section>
  );
}
